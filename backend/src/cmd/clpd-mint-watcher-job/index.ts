      ...
      for (const event of events) {
        try {
          ...
          if (matchingDeposit) {
            ...
            console.log(
              `❌ No matching deposit found for address ${tokenEvent.user} and amount ${tokenEvent.amount}`
            );
            await discordService.sendNotification(
...
async function checkUnmintedDeposits(): Promise<void> {
  ...
  if (unmintedDeposits.length >= UNMINTED_DEPOSITS_THRESHOLD) {
    ...
    if (
      !cooldownDoc.exists ||
      cooldownDoc.data()?.lastNotification.toDate() <
        new Date(now.getTime() - NOTIFICATION_COOLDOWN_HOURS * 60 * 60 * 1000)
    ) {
      await discordService.sendNotification(
